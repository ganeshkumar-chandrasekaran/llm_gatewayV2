...
class MockProvider(BaseProvider):
    """Offline provider that returns canned responses for testing without API keys."""
    name = "mock"
    capabilities = {
        "tools": True, "caching": False, "reasoning": False,
        "structured": True, "parallel_tools": False,
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        if response_format:
            ...
            for k, v in props.items():
                if k == "city":
                    ...
                    if "paris" in prompt_lower or "france" in prompt_lower:
                        obj[k] = "France"
                    else:
                        obj[k] = "Unknown"
                elif v.get("type") == "string":
        ...
        responses = {
            ...
            "say hello": "Hello! Greetings from Mock LLM!",
            "say hi": "Hi! I'm the mock provider.",
        }
        text = None
        for key, val in responses.items():
            if key in last_user.lower():
                ...
                break
        if text is None:
            text = f"[Mock LLM] You asked: \"{last_user[:100]}\" — This is a simulated response from the mock provider. Connect a real LLM provider for actual AI responses."
